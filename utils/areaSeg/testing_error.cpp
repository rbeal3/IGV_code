...
float * seg_info(pcl::PointCloud<pcl::PointXYZRGB>::Ptr incloud) {
	...
	std::cout  << " lowest x:" << info_array[0] << " highest x:" << info_array[1] << std::endl;
	//std::cout  << " lowest y:" << lowest_y << " highest y:" << highest_y << std::endl;
	std::cout  << " lowest x:" << info_array[4] << " highest x:" << info_array[5] << std::endl;
...
void passMap(pcl::PointCloud<pcl::PointXYZRGB>::Ptr incloud) {
	...
	for (float z=z_min; z<=z_max; z+=area){
		...
		info_array = seg_info(incloud);
		 ...
		 z_min = info_array[minBound_z];
		 z_max = info_array[maxBound_z];
		std::cout<< "Calling area_seg" << x_min <<" " << x_max <<" " << floor_min <<" " << floor_max<<" "<< z <<" " << z+area << std::endl;
		z_row_cloud =  area_seg(x_min, x_max, floor_min, floor_max, z, z+area, incloud, outname) ;

		pcl::PointCloud<pcl::PointXYZRGB>::Ptr test_pointer (&z_row_cloud);
		...
		//check array now
		std::cout<< "why the hell are these not the same???" << std::endl;
		std::cout << "info_array[minBound_x] " << info_array[minBound_x]  << " max: " << info_array[maxBound_x] <<std::endl;
		std::cout << "info_array[minBound_z] " << info_array[minBound_z]  << " max: " << info_array[maxBound_z] <<std::endl;
		...
		std::cout<< "calling area_seg" << x<<" " << x+area <<" " << floor_min <<" " << floor_max <<" " << z <<" " << z+area<<std::endl;
		x_partFrom_z_row = area_seg(x, x+area, floor_min, floor_max, z, z+area, z_row_pointer, outname) ;
